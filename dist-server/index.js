...
const hashPwd = async function (password, salt) {
  const buf = await pbkdf2Async(password, Buffer.from(salt, 'hex'), 100000, 32, 'sha256');
...
app.post('/api/employees/register', async function (req, res) {
  ...
  try {
    ...
    await execute("INSERT INTO KOSHA_EMPLOYEES (EMP_ID, PW_HASH, PW_SALT, EMP_NAME, STATUS, ORGANIZATION, DEPARTMENT, PHONE, CREATED_AT) VALUES (:1, :2, :3, :4, 'pending', :5, :6, :7, SYSTIMESTAMP)", [b.empId, hash, salt, b.empName, b.organization, b.department, b.phone]);
...
app.put('/api/employees/:empId', auth, adminOnly, async function (req, res) {
  ...
  try {
    if (b.status === 'rejected') {
      ...
      if (r.rowsAffected === 0) return res.status(404).json({
    ...
    if (r.rowsAffected === 0) return res.status(404).json({
      message: '직원을 찾을 수 없습니다. (ID: ' + empId + ')'
    });
    return res.json({
      message: '수정 완료'
    });
...
app.post('/api/employees', auth, adminOnly, async function (req, res) {
  const b = req.body;
  try {
    await execute("INSERT INTO KOSHA_EMPLOYEES (EMP_ID, PW_HASH, PW_SALT, EMP_NAME, STATUS, ORGANIZATION, DEPARTMENT, PHONE, CREATED_AT, APPROVED_AT) VALUES (:1, :2, :3, :4, :5, :6, :7, :8, SYSTIMESTAMP, SYSTIMESTAMP)", [b.empId, b.pwHash, b.pwSalt, b.empName, b.status || 'approved', b.organization, b.department, b.phone]);
    return res.status(201).json({
      message: '추가 완료'
    });
  } catch (err) {
  ...
  try {
    ...
    if (dupCheck.rows[0].CNT > 0) {
      return res.status(400).json({
        message: '이미 해당 월에 신청 내역이 있습니다.'
      });
    }
    const appId = generateUUID();
    await execute("INSERT INTO KOSHA_APPS (APP_ID, EMP_ID, APP_YEAR, APP_MONTH, ROOM_TYPE, NIGHTS, TOTAL, SUBSIDY, STATUS, CREATED_AT) VALUES (:1, :2, :3, :4, :5, :6, :7, :8, 'pending', SYSTIMESTAMP)", [appId, req.user.empId, year, b.month, b.roomType, b.nights, b.total, b.subsidy]);
    return res.status(201).json({
      id: appId
    });
  } catch (err) {
...
app.post('/api/apps/:id/cancel', auth, async function (req, res) {
  ...
  try {
    ...
    if (!appRecord) return res.status(404).json({
      message: '내역을 찾을 수 없습니다.'
    });
    if (appRecord.EMP_ID !== req.user.empId) return res.status(403).json({
      message: '권한이 없습니다.'
    });
    if (appRecord.STATUS === 'pending') {
      ...
      return res.json({
        message: '취소 요청이 접수되었습니다. 관리자 승인 후 최종 취소됩니다.'
      });
    } else {
      return res.status(400).json({
...
async function recalculateFundUsed() {
  try {
    ...
    console.error("Fund recalculation failed", err);
  }
}
