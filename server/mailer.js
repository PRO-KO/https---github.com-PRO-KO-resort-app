...
async function mailNewRegister(info) {
    ...
    return send({
        ...
        text: [
            ...
            '',
            '  사번     : ' + (info.empId || ''),
            '  소속기관  : ' + (info.organization || '미기재'),
            '  부서     : ' + (info.department || '미기재'),
            '',
            '※ 이 메일은 자동 발송되었습니다.'
        ].join('\n')
    });
...
async function mailAccountApproved(info) {
    ...
    return send({
        ...
        text: [
            ...
            '내부망 예약 시스템에 접속하여 예약을 신청하실 수 있습니다.',
            '',
            '감사합니다.',
            '안전보건공단 발전기금 사업팀 드림'
        ].join('\n')
...
async function mailLotteryResult(info) {
    ...
    return send({
        to: info.empEmail || internalEmailFor(info.empId),
        subject: isSelected
            ? '[휴양소 예약] 축하합니다! ' + info.month + '월 추첨 당첨 안내'
            : '[휴양소 예약] ' + info.month + '월 추첨 결과 안내',
        text: isSelected ? [
            (info.empId || '') + ' 님, 안녕하십니까.',
            '',
            info.month + '월 휴양소 예약 추첨에 당첨되셨습니다. 축하드립니다!',
            '',
            '  객실 유형  : ' + roomLabel,
            '  숙박 일수  : ' + info.nights + '박',
            '',
            ...
            (info.empId || '') + ' 님, 안녕하십니까.',
            '',
            info.month + '월 휴양소 예약 추첨 결과 다음 기회를 기약해 주시기 바랍니다.',
...
async function mailCancellation(info) {
    ...
    return send({
        ...
        text: [
            (info.empId || '') + ' 님, 안녕하십니까.',
            '',
            info.month + '월 휴양소 예약 신청이 취소 처리되었습니다.',
            '',
            '문의사항은 담당 부서로 연락 바랍니다.',
            '',
            '안전보건공단 발전기금 사업팀 드림'
